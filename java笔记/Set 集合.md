...
>为了让 `HashMap` 存取高效并减少碰撞，就需要确保数据尽量均匀分布。哈希值在 Java 中通常使用 `int` 表示，其范围是 `-2147483648 ~ 2147483647` 前后加起来大概 40 亿的映射空间，只要哈希函数映射得比较均匀松散，一般应用是很难出现碰撞的。但问题是，一个 40 亿长度的数组，内存是放不下的，所以，这个散列值是不能直接拿来用的。所以会先拿对象的 `hashCode()` 得到一个 int 值（可能是正数也可能是负数），然后对哈希表数组的长度取模（在Java 8及以后，通常用按位运算代替 `%`，比如 `(n - 1) & hash`，其中 n 是数组长度且一定是 2 的幂），得到的结果作为数组下标。

值得注意的是：取余（`%`）操作中如果除数是 2 的幂次则等价于与其除数减一的与（`&`）操作（也就是说 `hash % length == hash & (length - 1)` 的前提是数组长度 length 是 2 的 n 次方）。

- length = 8 时（1000），length - 1 = 7 的二进制位 `0111`
- length = 16 时（10000），length - 1 = 15 的二进制位 `1111`

也就是说当 `hash & 7` 时，只会考虑 `hash` 的最后三位，因为 7 只有三位有效位，所以 `hash` 前面的位与 0 做 & 等于 0，最后得出的结果也就是 `hash` 的最后三位的二进制；而对于 % 运算来说，`x % length` 实际上也就是只取 `x` 二进制的低 `n` 位，因为 `x = q * 2^n + r` 其中 `r` 是余数，满足 `0 <= r < 2^n`，如果 length 是 `2^n`，那么这个余数 `r` 刚好就是 `x` 的二进制中最低 `n` 位的值。所以这两种方法得到的最终结果是一致的，也证明了为什么需要设置 `HashMap` 的容量为2的次幂，但是对于计算机来说，取余操作对 CPU 更耗时。

>当数组容量从 `length` 扩容到 `length * 2` 时，元素的新位置只会因为 `hash` 中多出来的那一位（二进制）产生变化，8 -> 16 从 1000 -> 10000，对于某个元素计算新下标时 `hash & (length - 1)` 会关注第4位（从右数，0 开始），这导致元素要么位置保持不变，要么刚好移动到新数组后一半对应的位置，例如：

假设旧数组大小 `length = 8`，则 `length - 1 = 7 (0111)`，元素下标计算为 `hash & 7`，只看 `hash` 的后三位；扩容到 `length = 16`，则 `length - 1 = 15 (1111)`，计算新位置时看 `hash` 的四位（从右往左数）。如果新增的那一位是 `0`（第四位），则新位置和旧位置一样，如果是 `1`，新位置则在旧位置基础上加上了 `8`（正好是旧容量），即对应到新数组的后半部分。也就是说扩容时不用重新计算所有元素的 `hash`，只需要检查 `hash` 新增的一位即可决定元素是否移动到新数组位置。

![](images/Set%20集合/file-20250429155613.png)

当然采用二进制位操作 `&` 还可以让 `HashMap` 在扩容的时候更均匀。因为底层计算哈希表的下标用的是 `(table.length - 1) & hash` ，因为前面有设置哈希表的大小一定是 2 的次幂，所以`(table.length - 1)` 一定是一个奇数，即二进制位的最低位一定是 1 ，而 `hash` 值可奇可偶，即二进制最低位可为 1 或 0，通过与运算后得到的下标位置可以较为均匀的分配为奇或偶数（不会全是奇或偶），可以较好地减少哈希冲突
