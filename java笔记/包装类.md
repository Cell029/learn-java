...
>Java 编译器在编译阶段，会将代码中的装箱/拆箱语法糖转换成显式的方法调用。这些方法主要是包装类的 `valueOf()` 和 `xxxValue()`。

```java
int num = 10;

Integer i = num;        // 自动装箱：int → Integer
                        // 编译后等于Integer obj = Integer.valueOf(num)
int val = i;            // 自动拆箱：Integer → int
                        // 编译后等于int val = obj.intValue()
```
...
但需要注意的是，拆箱时可能出现空指针异常：

```java
Integer a = null;
// 自动拆箱为 a.intValue()，但此时 a 是 null，所以报错
...
>Java 为了优化内存和性能，对于频繁使用的小整数对象（`Integer` 类型）进行了缓存机制，Java 默认缓存的是 -128 到 127 之间的整数对象
...
| 对比项     | 基本数据类型（primitive）         | 包装类型（wrapper）                   |
| ------- | ------------------------- | ------------------------------- |
| 定义方式    | `int, double, char...`    | `Integer, Double, Character...` |
| 是否是对象   | 不是对象                      | 是对象                             |
| 是否可用于泛型 | 不可直接使用                    | 可用于泛型                           |
| 默认值     | `int=0, boolean=false...` | `null`                          |
| 存储位置    | 局部变量 → 栈中成员变量 → 堆中        | 通常分配在堆中（有例外）                    |
| 比较方式    | `==` 比较值                  | `==` 比较地址，`equals()` 比较值        |
| 作用场景    | 高性能、低内存场景                 | 面向对象、需要泛型、集合等场景                 |
