...
>
>当一个线程正在访问一个对象的 synchronized 实例方法时，其他线程不能访问该对象的所有synchronized 方法（当一个线程获取了该对象的锁之后，其他线程无法获取该对象的锁），如果另一个线程使用的是另一个对象，则可以使用 synchronized 方法，因为两个实例对象的锁并不同相同
...
>作用范围为整个方法题，锁对象是这个类的 `.class` 对象（也叫类锁），所以通过该类创建的所有对象共享同一个锁，存在互斥
...
>但静态 synchronized 方法 和 非静态 synchronized 方法加锁互不干扰
