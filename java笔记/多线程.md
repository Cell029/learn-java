...
>对于 `synchronized` 方法，并不是依靠 monitorenter 和 monitorexit 指令实现的，被 javap 反汇编后可以看到，synchronized 方法和普通方法大部分是一样的，不同在于，这个方法会有一个叫作 ACC_SYNCHRONIZED 的 flag 修饰符，代表需要 Monitor 支持
...
- 如果线程池核心线程数已满（即线程数已经等于 `corePoolSize` ），一个新提交的任务会被放进任务队列 `workQueue` 排队等待执行。
- 当线程池里面存活的线程数已经等于 `corePoolSize` 了，并且任务队列 `workQueue` 也满，判断线程数是否达到 `maximumPoolSize`（即最大线程数是否已满），如果没到达，则创建一个非核心线程执行提交的任务。
