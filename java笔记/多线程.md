...
- 当 Thread-2 执行 synchronized(obj) 时就会将 Monitor 的所有者 Owner 置为 Thread-2，Monitor 中只能有一个 Owner

- 在 Thread-2 上锁的过程中，如果 Thread-3，Thread-4，Thread-5 也来执行 synchronized(obj)，就会进入 EntryList 

- Thread-2 执行完同步代码块的内容，然后唤醒 EntryList（阻塞队列） 中等待的线程来竞争锁，竞争是非公平的

- 图中 WaitSet 中的 Thread-0，Thread-1 它们调用了 `wait()` 方法，已经释放锁，进入 `WAITING` 状态（所有使用 `wait()` 方法的线程都会进入这里）
