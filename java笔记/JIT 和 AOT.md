...
public static void methodEscape() {
	Object obj = new Object(); // 原本是局部变量
	list.add(obj); // 引用传到外部，全局 list 中
}
```
...
**1、常规路径：解释器 -> C1（2层）-> C2（4层）**

>大多数热点方法按正常路径执行：先由解释器执行 -> 再进入带 profiling 的 C1（第 2 层） -> profiling 数据充分后，交由 C2 优化编译

**2、简单方法：解释器 -> C1（2层）-> 放弃 C2，退回用 C1（1层）**
...
**4、C2 忙碌：解释器 -> C1（1层）-> C1（2层）**

>当 C2 编译器繁忙（或被限制）时，JVM 为了避免长时间停留在解释器执行，会先用较低层级的 C1（1层）做初步编译，待条件允许后再进入 C1（2层），为后续 C2 编译做准备
