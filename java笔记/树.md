...
public class BinaryTree {  
    ...
    public TreeNode insert(TreeNode node, int value) {  
        ...
        if (value < node.value) {  
            ...
            System.out.println("节点 " + value + " 已存在，跳过插入");   
        }  
        return node;  
    }  
    ...
    private TreeNode deleteRec(TreeNode node, int value) {  
        ...
        if (value < node.value) {  
            // 当递归删除后，原来的 node.left 节点可能会被替换为它的子节点，或者变成 null            // 所以要用 node.left 来接收变化后的结果  
            ...
            // 该节点有两个孩子  
            TreeNode minNode = findMin(node.right); // 找到右子树中的最小值  
...
System.out.println("中序遍历（有序输出）：");  
tree.inOrder(tree.root);  // 20 30 40 50 60 70 80  
System.out.println();  
  
System.out.println("树的高度：" + tree.getHeight(tree.root));  // 3  
System.out.println("节点总数：" + tree.countNodes(tree.root)); // 7  
  
System.out.println("\n查找元素：");  
System.out.println("是否存在 60 ？" + tree.search(tree.root, 60)); // true  
System.out.println("是否存在 90 ？" + tree.search(tree.root, 90)); // false  
...
2、删除的是只有一个子节点的节点，让子节点顶替当前节点

3、删除的是有两个子节点的节点，找到右子树中“最小的节点”或左子树中“最大的节点”来替换当前节点的值，然后递归删除该节点
...
>当新插入的节点的父节点和叔节点都是红色时，直接修改叔父爷的节点颜色，然后爷作为新子树的根节点，如果是整棵树的根节点，则在插入过程的最后会强制将根节点置为黑色
...
![](images/树/c902ed14c1f68d0da71c3f4b45c61596.png)

2.3 LR 型：左旋右旋，儿、爷染色

![](images/树/dd9447c97ac2b06b6dc75fff6ec44a83.png)

>这里儿指的是 9（即插入的新节点），爷指的是 10

2.4 RL 型：右旋左旋，儿、爷染色

![](images/树/08ad62ffd9ac3d2b084c478389264ac8.png)

****
## 3.2 删除逻辑

- 按照普通 BST 删除逻辑删除节点（分三种情况：无子、单子、双子）
- 若被删除节点是红色，无需处理
- 若被删除节点是黑色，则可能破坏黑色平衡，需要修复
- 修复过程主要处理 “双黑”问题：路径中黑色节点少了一个

![](images/树/file-20250430155028.png)
...
>只要远端的是红，不管近端有没有孩子，都可以看作是 LL 或者 RR 型

![](images/树/6baee4eb9205f864bf4f24aa7ddf81c9_720.png)

****
###### 1.2 兄弟的近端孩子是红

![](images/树/55f567ac1b9bee1185b8d8361cf3e6c3.png)
...
![](images/树/a8d46dc2c837d7fb985ee75b4d199954_720.png)

>这种就是上移后兄弟节点属于 RR 类型

****

![](images/树/0239cf4b23f3ad9af99623afde8867b9.png)

>双黑上移碰到红色节点，直接变成黑色节点就行

>如果兄弟变红双黑上移后碰到根节点，直接结束就行
...
![](images/树/d13dacaa0f1d41c0a584abe9b9de75fc.png)
