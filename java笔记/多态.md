...
>这个也叫静态多态,在编译阶段确定,叫做方法重载，Java允许在一个类中定义多个相同方法名的方法,前提是需要形式参数列表不同(类型,顺序,个数),这就是方法重载
...
>子类重写方法时,可以把父类的返回值类型修改成返回值类型的子类,但是必须是引用型的返回值类型（对象类型）才行（协变返回类型），不适用于基本数据类型（不存在继承关系）
...
>在编译阶段,编译器只知道animal的类型是Animal类型,所以编译时就会去 Animal 类中找 move 方法,找到后就绑定该方法的符号引用,并且编译阶段就决定当前的引用能够使用什么方法,那就是使用编译类型的方法,想要使用实际对象的方法就要向下转型,所以向上转型是有一定的缺陷的
...
**为什么 `private`, `static`, `final` 会破坏多态？**

- **`private`**：方法是私有的，不存在继承，更谈不上重写。所有 `private` 方法的调用在编译期就通过 `invokespecial` 指令确定好了，与对象实际类型无关。
- **`static`**：方法是属于类的，而不是对象的。调用哪个类的静态方法，完全取决于引用变量的编译时类型。通过 `Animal.myStaticMethod` 和 `Dog.myStaticMethod` 调用的是两个不同的方法，使用 `invokestatic` 指令。
- **`final`**：`final` 方法明确告诉JVM：“我不允许被重写，我的实现是最终版本”。因此，JVM和编译器都可以对其进行优化，比如静态解析（编译期绑定），因为它确定不会在运行时改变。
...
>这里存在一个调用逻辑的问题,如果不使用向上转型，那么每次使用某个对象的某个方法都需要去创建这个对象然后调用这个对象的方法，虽然向上转型后也需要创建不同的对象，但是每个对象的引用类型是统一的，再根据编译时的字符引用的特性，就可以直接使用这个向上转型的类里面的方法，哪怕它在底层真正调用的是具体对象的重写的方法，不管创建多少个对象，调用的方法始终是同一个方法，只不过传入的引用是不同的而已。
...
**如果子类中定义了一个与父类方法签名完全相同的静态方法，那么这个方法不会重写父类的静态方法，而是隐藏了它， 静态方法的调用是在编译期就由编译器决定的，取决于声明变量时的类型，与实际对象类型无关，JVM 在运行时不会进行多态选择** ^e66367
