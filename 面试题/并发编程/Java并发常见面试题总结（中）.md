...
为什么这样设计呢，因为这样可以防止死锁和逻辑混乱，首先需要明确一下读写锁的升降级概念。锁升级指的是线程先持有读锁，然后尝试获取写锁，读锁是共享的，多个线程可能同时持有读锁，而写锁是独占的，如果线程持有读锁时尝试获取写锁，可能会导致死锁（因为其他线程也可能持有读锁），所以不允许读锁升级为写锁，也就是不能获取写锁。锁降级指的是线程先持有写锁，然后获取读锁，写锁独占，并且当前线程已经持有写锁的情况下，获取读锁时不会产生冲突（因为写锁已经独占了访问权），这是安全的，而且常用在 “修改-读取” 场景中：先修改共享资源，再降级为读锁让其他线程读取。不管怎样，本质上一个线程还是只获取一个锁在控制访问。

```java
writeLock.lock(); // 阶段1：获取写锁（独占）
try {
    updateData(); // 独占写操作
    readLock.lock(); // 阶段2：获取读锁（准备降级）
} finally {
    writeLock.unlock(); // 阶段3：释放写锁，现在只持有读锁
}
// 现在处于阶段4：只持有读锁（共享）
```
...
---------------------------------------------------
t0:          获取写锁                 其他线程阻塞
t1:          持有写锁，执行写操作      其他线程阻塞  
t2:          获取读锁                 其他线程阻塞（因为写锁还在）
t3:          释放写锁                 其他线程可以获取读锁
t4:          持有读锁，执行读操作      其他线程可以并发读
```
...
****
## 7.3 StampedLock 适合什么场景

和 `ReentrantReadWriteLock` 一样，`StampedLock` 同样适合读多写少的业务场景，可以作为 `ReentrantReadWriteLock` 的替代品，而且性能更好。不过需要注意的是：`StampedLock` 不可重入，不支持条件变量  `Condition`，对中断操作支持也不友好（使用不当容易导致 CPU 飙升）。如果你需要用到 `ReentrantLock` 的一些高级性能，就不太建议使用 `StampedLock` 了。
