...
线程池是一种多线程处理机制，它在程序启动或运行过程中会预先创建一定数量的线程并将其保存在池中，当有任务到来时不需要重新创建线程，而是直接从池中取出空闲线程来执行任务，执行完任务后线程不会被销毁，而是重新归还到池中等待下一个任务。这样避免了频繁创建和销毁线程带来的性能开销，还可以通过统一管理线程数量来防止系统因为并发过高而资源耗尽。
...
2. **提高响应速度**：因为线程池里通常会维护一定数量的核心线程，任务来了之后就可以直接交给这些已经存在的、空闲的线程去执行，省去了创建线程的时间，任务能够更快地得到处理。
3. **提高线程的可管理性**：线程池允许统一管理池中的线程，可以配置线程池的大小（核心线程数、最大线程数）、任务队列的类型和大小、拒绝策略等。这样就能控制并发线程的总量，防止资源耗尽，保证系统的稳定性。同时，线程池通常也提供了监控接口，方便了解线程池的运行状态（比如有多少活跃线程、多少任务在排队等），便于调优。
