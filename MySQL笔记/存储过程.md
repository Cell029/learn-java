...
>触发器是一种特殊的存储过程，它不需要手动调用，而是在对表执行某些操作时自动执行，通过触发器可以在数据变更时自动执行相关的逻辑，比如：
...
- `BEFORE` / `AFTER`：表示触发器是在操作之前还是之后执行；
- `INSERT | UPDATE | DELETE`：指定触发器响应的操作类型；
- `FOR EACH ROW`：表示每一行执行一次（MySQL 目前仅支持行级触发器）；
...
CREATE TRIGGER users_insert_before
BEFORE INSERT 
ON users
FOR EACH ROW

	BEGIN
		insert into `register_log` values(NOW(),"NEW YORK","WINDOWS");
	END //
		
DELIMITER ;
```

>当使用 `INSERT` 语句后就会在 register_log 表中插入一条数据：

![](images/存储过程/file-20250522143401.png)

****
## 10.1 触发器的 NEW、OLD 关键字

>可以通过 `NEW` 和 `OLD` 关键字来访问当前操作的数据记录，包括插入的新数据、更新前后的数据、被删除的数据等；
>
>`NEW`：表示新数据，即正在插入或更新的值； `OLD`： 表示旧数据，即被删除或更新前的值

- `INSERT`: `OLD` 不可用，`NEW` 可用（表示当前插入的这条行数据）
- `UPDATE`: `OLD` 可用（表示修改前的老数据），`NEW` 可用（表示修改后的新数据）
- `DELETE`: `OLD` 可用（表示删除前的老数据），`NEW` 不可用
...
BEGIN
  ...
  IF old_name != new_name THEN
    -- 插入日志记录
    INSERT INTO user_update_log (user_id, old_username, new_username, update_time)
    VALUES (OLD.id, old_name, new_name, NOW());
  END IF;
END //
...
BEGIN
  -- 将被删除用户的信息插入日志表
  INSERT INTO user_delete_log (user_id, username, email, delete_time)
...
- `SELECT * FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = '数据库名';`：查看`MySQL`已定义的触发器（去掉 `WHERE` 就是查看全部）
- `DROP TRIGGER IF EXISTS 触发器名称;`：删除某个指定的触发器（MySQL 不支持修改触发器，只能通过删除重建达到修改的目的）
